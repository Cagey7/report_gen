...
class TradeDataPreparer:
    ...
    def prepare(self):
        ...
        data_for_doc = {}
        data_for_doc["document_header"] = f"Взаимная торговля {region_cases[self.region]['родительный']} с {country_cases[self.country_or_group]['творительный']} за {month_ranges[month]}{self.year} {'год' if month == 12 else 'года'}"
        ...
        data_for_doc["file_name"] = f'Справка по торговле {region_cases[self.region]["родительный"]} с {country_cases[self.country_or_group]["творительный"]} ({month_ranges[month]}{self.year} {"год" if month == 12 else "года"}).docx'
        return data_for_doc
